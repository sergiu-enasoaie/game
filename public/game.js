...
const gameSizeX = 1500; // will be downscaled 5x to 300
const gameSizeY = 1900; // will be downscaled 5x to 480
...
function isValidPosition(newPosition, playerId) {
  // bounds check
  if (newPosition.x < 0 || newPosition.x + playerSize > gameSizeX) {
    return false
  }
  if (newPosition.y < 0 || newPosition.y + playerSize > gameSizeY) {
    return false
  }
  // collision check
...
function shuffleDoubloon() {
  var posX = Math.floor(Math.random() * Number(gameSizeX) - 100) + 10
  var posY = Math.floor(Math.random() * Number(gameSizeY) - 100) + 10

  while (!isValidPosition({ x: posX, y: posY }, '_doubloon')) {
    posX = Math.floor(Math.random() * Number(gameSizeX) - 100) + 10
    posY = Math.floor(Math.random() * Number(gameSizeY) - 100) + 10
  }
